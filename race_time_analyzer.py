#import all the functions from the race_time_functions.py to be avaialable for use here
from race_time_functions import *

print("\n🏁 Race Time Analyzer\n")

# Ask for number of races (integer > 0), with validation

num_races = get_valid_positive_int("Enter number of races: ")
...
while race_count <= num_races:
    ...
    while time_count <= num_times:
        ...
        valid_time = get_valid_positive_float("Enter the time: ")
    ...
    race_avg = get_and_calculate_average (race_total,time_count - 1)#-1 due to using time_count in prompt
